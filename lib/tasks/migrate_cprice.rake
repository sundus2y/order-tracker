namespace :migrate do
  desc 'Migrate CPrice'
  task cprice: :environment do
    workbook = Roo::Spreadsheet.open("lib/tasks/#{ENV['FILE']}.xlsx")
    raw_data = []
    error_list = []
    header = %w(item_number name c_price)
    header_hash = {}
    ...
    workbook.sheet(0).each_with_index(header_hash) do |hash|
      begin
        ...
        hash['c_price'] = (hash['c_price'] || 0).to_f
        raw_data << hash
      rescue Exception => e
        error_list << hash
      end
    ...
    raw_data.in_groups_of(500).each_with_index do |raw_data_group,index|
      begin
        ...
        if new_list.count > 0
          new_sql = 'INSERT INTO items (item_number, name, original_number, c_price, brand, made) VALUES '
          new_sql += new_list.map do |new_item|
            "('#{new_item['item_number']}', #{ActiveRecord::Base.sanitize(new_item['name'])}, '#{new_item['item_number']}', '#{new_item['c_price']}', 'HYUNDAI', 'KOREA')"
          end.join(', ')
          i_result = ActiveRecord::Base.connection.execute(new_sql)
          insert_sql_result = i_result.cmd_status
        end
        if update_list.count > 0
          update_sql = 'UPDATE items AS i SET c_price = u.c_price FROM (values'
          update_sql += update_list.map do |update_item|
            "('#{update_item['item_number']}', #{update_item['c_price']})"
          end.join(', ')
          update_sql += ') AS u(item_number, c_price) WHERE u.item_number = i.item_number'
