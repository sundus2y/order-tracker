class SearchesController < ApplicationController
  ...
  def items
    ...
    results = Item.joins(:inventories).includes({inventories: :store}, :order_items).where(search_query).reorder(updated_at: :desc).limit(30).as_json({type: search_type}) if params[:inventory].present?
